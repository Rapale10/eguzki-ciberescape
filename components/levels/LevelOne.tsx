...
import { getLevelFeedback } from '../../services/geminiService';
import { Search, X, Monitor, HelpCircle } from 'lucide-react';
import { COLORS, LEVEL_TIPS } from '../../constants';
...
export const LevelOne: React.FC<LevelOneProps> = ({ onComplete, onExit }) => {
  ...
  const checkPassword = async () => {
    ...
    if (isCorrect) {
      ...
      // Generate dynamic feedback via Gemini or fallback, but show it in a Modal
      const aiMsg = await getLevelFeedback(false, "user failed to guess password based on desk clues");
      ...
      setModalState({
          ...
          message: `Contraseña incorrecta. Pista del sistema: ${aiMsg}`,
